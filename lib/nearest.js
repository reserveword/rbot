var bot, isNotEmpty, vec3, io;

function init(_bot, _isNotEmpty, _blocks, _vec3, _io) {
    bot = _bot;
...
function nearestBlock_(blockName) {
    ...
    var dmax = new vec3(64, 64, 64),
        block, dmin = 100000000,
        bmin = null,
        d;
    var x, y, z;
...
function nearestBlock(blockName, done) {
    ...
    bot.findBlock({
        ...
        if (blockPoints.length) {
            ...
            io.verbose("I couldn't find any " + blockName + " blocks within 256.");
            done(null);
            return;
        }
    });
