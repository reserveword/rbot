...
function init(_bot, _vec3, _achieve, _achieveList, _processMessage, _mf, _async, _io) {
    io = _io;
    async = _async;
    ...
    inventory.init(bot, mf.materials, io);
    nearest.init(bot, isNotEmpty, blocks, vec3, io);
    stringTo.init(bot, inventory, nearest, vec3, isEmpty, isNotEmpty, blocks, async, io);


    syntaxTask = require('./task/syntaxTask');
    syntaxTask.init(achieve, achieveList, stringTo);
    moveTask = require('./task/moveTask');
    moveTask.init(bot, processMessage, isEmpty, stringTo, vec3, isNotEmpty, io);
    inventoryTask = require('./task/inventoryTask');
    inventoryTask.init(bot, stringTo, findItemType, inventory, vec3, io);
    blockTask = require('./task/blockTask');
    blockTask.init(bot, stringTo, isNotEmpty, isBlockEmpty, isBlockNotEmpty, isEmpty, vec3, positionToString, processMessage, io);
    informationTask = require('./task/informationTask');
    informationTask.init(bot, stringTo, io);
