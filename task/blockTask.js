var bot, stringTo, isNotEmpty, isBlockEmpty, isBlockNotEmpty, isEmpty, vec3, processMessage, positionToString, io;

function init(_bot, _stringTo, _isNotEmpty, _isBlockEmpty, _isBlockNotEmpty, _isEmpty, _vec3, _positionToString, _processMessage, _io) {
    bot = _bot;
    stringTo = _stringTo;
...
function watch(ent, done) {
    ...
    bot.on('blockUpdate', function(firstPosition) {
        wa = [done, function(oldBlock, newBlock) {
